...
- **樹苗有臉，而且刻意跟喝水的水滴同一套語言**：`#1E2A22` 的圓點眼睛、
  ...
  不是一個介面表面
- **角色是一顆有臉的豆子，而且豆子永遠不改大小。** 長大的是上面的東西：
  一對小葉 → 一對大葉 → 兩對（豆苗）→ 樹冠（小樹）→ 滿樹花（開花）。
  豆子固定不變有兩個理由：進度一眼看得出（葉子對數可以數），而且臉在每個階段
  都一樣清楚。曾經試過讓身體隨階段長大，結果臉會跟著脹大、階段之間反而難分辨
...
- **早晨問候／晚間總結**：`lib/coach.js` ＋ `components/DailyCoach.jsx`。
  早上 11 點前顯示問候，晚上 7 點後顯示總結，中間什麼都不顯示（那個時段三行
  明細講得比任何文案清楚）。可以關掉，關掉的狀態記在 `coach-dismissed`
